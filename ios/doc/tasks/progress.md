...
- The valve bridge intentionally keeps a lightweight `window.wx` stub plus `wxMark` flag because the current Android source-of-truth does the same; this contradicts the original requirement wording that said not to mock the WeChat bridge, so the code now follows Android behavior and the contradiction is documented rather than hidden.
